...
const HomeBody6 = ({ products }: { products: ProductTrue[] }) => {
  ...
  const handleMouseDown2 = (e: React.MouseEvent) => {
    ...
    setIsDragging(true);
    setStartX(e.clientX);
    setScrollLeftStart(scrollContainerRef2.current.scrollLeft);

    // Change cursor to grabbing
    scrollContainerRef2.current.style.cursor = "grabbing";
    scrollContainerRef2.current.style.userSelect = "none";
  };
  ...
  const handleMouseMove2 = (e: React.MouseEvent) => {
    if (!isDragging || !scrollContainerRef2.current) return;

    e.preventDefault();
    const x = e.clientX;
    const walk = (x - startX) * 1.5; // Adjust multiplier for scroll speed

    scrollContainerRef2.current.scrollLeft = scrollLeftStart - walk;
  };
  ...
  const handleMouseUp2 = () => {
    ...
    setIsDragging(false);
    scrollContainerRef2.current.style.cursor = "grab";
    scrollContainerRef2.current.style.removeProperty("user-select");
  };
  ...
  return (
    <div className="relative px-4 py-8 max-w-[100vw] xl:max-w-7xl">
      <h2 className="text-2xl font-bold mb-6">Explorer nos produits</h2>
