...
const ProductCard: React.FC<{
  ...
  /* 
${
        index === 0 ? "w-64 h-80" : index === 1 ? "w-64 h-40" : "w-32 h-40"
      }

      ${
          index === 0 ? " h-40" : index === 1 ? " h-20" : "h-20"
        }
*/
    ...
    return (
      <div
        className={`flex-shrink-0 w-full  ${
          index === 0 ? " h-80" : index === 1 ? " h-40" : " h-40"
        } bg-gray-200 rounded-lg overflow-hidden animate-pulse`}
      >
  ...
  return (
    <div
      className={`relative flex-shrink-0 w-full  ${
        index === 0 ? " h-80" : index === 1 ? " h-40" : " h-40"
      }  bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 mx-2`}
...
const HomeBody7 = ({ products }: { products: ProductTrue[] }) => {
  /*  const [products, setProducts] = useState<Product[]>([]); */
  const [loading, setLoading] = useState(false);
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const [productsNews, setProductsNews] = useState<ProductTrue[]>([]);
  const [isDragging, setIsDragging] = useState(false);
  const [startX, setStartX] = useState(0);
  const [scrollLeftStart, setScrollLeftStart] = useState(0);
  useEffect(() => {
    /*  const getProductsNews = async () => {
    
      setProductsNews(product);
    }; */
  ...
  return (
    <div className="relative px-4 py-8 max-w-[100vw] xl:max-w-7xl">
      <h2 className="text-2xl font-bold mb-6">Nouveauté</h2>
      ...
      <div className="relative">
        <div
          ...
          className=""
        >
          <div className=" grid grid-rows-2 grid-cols-4 gap-2 space-x-4">
