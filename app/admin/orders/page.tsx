...
interface initialStateOrdersType {
  loading: boolean;
  error: string | orderType[];
  orders: string | orderType[];
}

interface actionOrdersType {
  type: string;
  payload: string | orderType[];
}

function reducer(state: initialStateOrdersType, action: actionOrdersType) {
  switch (action.type) {
    case "FETCH_REQUEST": {
